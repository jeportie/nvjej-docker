...
local function create(opts)
  ...
  -- Determine shell and prepare command with interactive flag.
  local shell = vim.o.shell  -- Ensure this is your zsh binary, e.g. "/bin/zsh"
  local cmd
  if opts.cmd and opts.buf then
    cmd = { "zsh", "-i", "-c", format_cmd(opts.cmd) .. "; zsh -i" }
  else
    cmd = { "zsh", "-i" }
  end
